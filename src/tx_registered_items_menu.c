...
#define TAG_ITEM_ICON       5112
//#define TILE_TAG_INDICATOR_ARROWS  5111
#define TAG_SCROLL_ARROW    5114
...
static void TxRegItemsMenu_RefreshListMenu(void)
{
    ...
    for(i = 0; i < TxRegItemsMenuItemPageInfo.count - 1; i++)
    {
        ...
        gTxRegItemsMenu->listItems[i].label = &(gTxRegItemsMenu->itemNames[i][0]);
        gTxRegItemsMenu->listItems[i].index = i;
    }
    StringCopy(&(gTxRegItemsMenu->itemNames[i][0]) ,gFameCheckerText_Cancel);
    gTxRegItemsMenu->listItems[i].label = &(gTxRegItemsMenu->itemNames[i][0]);
    gTxRegItemsMenu->listItems[i].index = -2;
    ...
    gMultiuseListMenuTemplate.item_X = 48;
    gMultiuseListMenuTemplate.cursor_X = 40;
    ...
    gMultiuseListMenuTemplate.moveCursorFunc = TxRegItemsMenu_MoveCursor;
    gMultiuseListMenuTemplate.itemPrintFunc = TxRegItemsMenu_PrintFunc;
...
static void TxRegItemsMenu_MoveCursor(s32 id, bool8 b, struct ListMenu *thisMenu)
{
    ...
    if (gTxRegItemsMenu->toSwapPos == 0xFF)
    {
        ...
        if (id != -2)
            ...
            TxRegItemsMenu_PrintItemIcon(375);
    }
}
...
static void TxRegItemsMenu_StartScrollIndicator(void)
{
    if (TxRegItemsMenuItemPageInfo.scrollIndicatorTaskId == TASK_NONE)
        TxRegItemsMenuItemPageInfo.scrollIndicatorTaskId = AddScrollIndicatorArrowPairParameterized(SCROLL_ARROW_UP, 28, 110, 148, TxRegItemsMenuItemPageInfo.count - TxRegItemsMenuItemPageInfo.pageItems, TAG_SCROLL_ARROW, TAG_SCROLL_ARROW, &(TxRegItemsMenuItemPageInfo.itemsAbove)); //176, 12, 148 x, y1, y2
}
...
static void TxRegItemsMenu_RemoveItemIcon(void) //remove item storage selected item icon
{
    ...
    if (Overworld_GetFlashLevel() > 0 && *spriteId2Loc != SPRITE_NONE)
    {
        FreeSpriteTilesByTag(TAG_ITEM_ICON);
        FreeSpritePaletteByTag(TAG_ITEM_ICON);
